...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        ######################## DATA WEIGHTING ########################
        def get_ensemble_lprobs_with_weight(x, dim, keepdim, distribution):
            x = torch.exp(x)
            x = x * distribution
            return torch.log(torch.sum(x, dim, keepdim=keepdim))

        distribution = torch.tensor(
            # [0.6, 0.3, 0.1]
            [0.65, 0.25, 0.1]
        ).unsqueeze(1).unsqueeze(2).cuda(log_probs[0].device)
        # logging.info(f"ensemble weight: {distribution}")
        avg_probs = get_ensemble_lprobs_with_weight(
            torch.stack(log_probs, dim=0), dim=0, keepdim=False, distribution=distribution)
        attns = torch.stack(attns, dim=0)
        attns = attns * distribution
        if avg_attn is not None:
            avg_attn = torch.sum(attns, dim=0, keepdim=False)
