...
class Conv1dAdaptor(nn.Module):
    ...
    def add_args(cls, parser):
        parser.add_argument("--adaptor-in-dim", type=int)
        parser.add_argument("--adaptor-out-dim", type=int)
...
class Wav2VecEncoderWithAdaptor(FairseqEncoder):
    def build_adaptor(self, args):
        ...
        if args.adaptor_n_layers > 0:
            adaptor = Conv1dAdaptor(
                args.adaptor_in_dim, # args.decoder_embed_dim,
                args.adaptor_out_dim, # args.decoder_embed_dim,
    ...
    def __init__(self, args):
        super().__init__(None)
        if args.pretrain_encoder_name == 'wav2vec2':
            self.w2v_encoder = Wav2VecEncoder(args)
        elif args.pretrain_encoder_name == 'hubert':
            self.w2v_encoder = HubertEncoder(args)
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument("--pretrain-encoder-name", default='wav2vec2')
...
class XMTransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_encoder(cls, args):
        # args.pretrain_encoder_name = "wav2vec2"
        # if args.pretrain_encoder_name == "wav2vec2":
        if args.pretrain_encoder_name == "wav2vec2" or args.pretrain_encoder_name == "hubert":
...
def set_default_adaptor_args(args):
    args.adaptor_in_dim = getattr(args, "adaptor_in_dim", 1024)
    args.adaptor_out_dim = getattr(args, "adaptor_out_dim", 1024)
...
@register_model_architecture(model_name="xm_transformer", arch_name="xm_transformer_hubert_base")
def base_hubert_architecture(args):
    args.max_positions = getattr(args, "pretrain_encoder_name", "wav2vec2")
    args.adaptor_in_dim = getattr(args, "adaptor_in_dim", 768)
    args.adaptor_out_dim = getattr(args, "adaptor_out_dim", 1024)
    base_architecture(args)


@register_model_architecture(model_name="xm_transformer", arch_name="xm_transformer_hubert_l")
def base_hubert_architecture(args):
    args.max_positions = getattr(args, "pretrain_encoder_name", "wav2vec2")
    args.adaptor_in_dim = getattr(args, "adaptor_in_dim", 1024)
    args.adaptor_out_dim = getattr(args, "adaptor_out_dim", 1024)
...
@register_model_architecture(model_name="xm_transformer", arch_name="xm_transformer_hubert_xl")
def base_hubert_architecture(args):
    args.max_positions = getattr(args, "pretrain_encoder_name", "wav2vec2")
    args.adaptor_in_dim = getattr(args, "adaptor_in_dim", 1280)
    args.adaptor_out_dim = getattr(args, "adaptor_out_dim", 1024)
    base_architecture(args)
